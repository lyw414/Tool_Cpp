...
namespace LYW_CODE
{
    ...
    public:
        ...
        int push_back(void * Data )
        {
            ...
            if ( next != beginIndex )
            {
                memcpy ( m_DataArray + endIndex * m_DataSize, Data, m_DataSize );
                endIndex = next;
                return 1;
            }
            else
        ...
        int pop_front(void * Data)
        {
            ...
            if ( next != endIndex )
            {
                memcpy ( Data, m_DataArray + next * m_DataSize, m_DataSize );
                beginIndex = next;
                return 1;
            }
            else
        ...
        int push_all(void * DataArray, int ArraySize)
        {
            ...
            if ( endIndex < beginIndex )
            {
                ...
                if ( num >= ArraySize )
                {
                    memcpy ( m_DataArray + endIndex * m_DataSize, DataArray, ArraySize * m_DataSize );
                    endIndex += ArraySize;
                    return 0;
                }
                else
                {
                    memcpy ( m_DataArray + endIndex * m_DataSize, DataArray, num * m_DataSize );
            ...
            else
            {
                ...
                {
                    memcpy ( m_DataArray, DataArray + num * m_DataSize, ( ArraySize - num ) * m_DataSize ) ;
                    endIndex =  ( ArraySize - num );
                    return 0;
                }
                else
                {
                    memcpy ( m_DataArray, DataArray + num * m_DataSize, ( beginIndex - 1 ) * m_DataSize ) ;
                    endIndex = beginIndex - 1;
        ...
        int pop_all(void * DataArray)
        {
            ...
            if ( endIndex > beginIndex  )
            {
                ...
                memcpy ( DataArray, m_DataArray + ( beginIndex + 1 ) * m_DataSize,num * m_DataSize );
            ...
            else
            {
                ...
                memcpy ( DataArray, m_DataArray + ( beginIndex + 1 ) * m_DataSize,num * m_DataSize );
                memcpy ( DataArray + num * m_DataSize, m_DataArray, endIndex * m_DataSize );
                num += endIndex ;
                beginIndex = endIndex - 1;
                return num;
            }
